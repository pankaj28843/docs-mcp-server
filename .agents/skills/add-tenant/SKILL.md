...
### Apple Developer Documentation / DocC exception

If the target is `https://developer.apple.com/documentation/`, use the
`apple-docc-tenant` skill instead of a normal online tenant. Apple Developer
Documentation renders a JavaScript DocC shell; browser evidence should show
`Fetch` requests to `/tutorials/data/documentation.json` and
`/tutorials/data/documentation/technologies.json`. Maintain it as a filesystem
tenant generated by `scripts/apple_docc_snapshot.py`, then rebuild the index and
