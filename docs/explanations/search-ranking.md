...
| `body` | text | yes | yes | 1.0 | Full content (stored for snippets) |
...
- `mmap_size = 134217728` (128MB) ([https://www.sqlite.org/mmap.html](https://www.sqlite.org/mmap.html))
- `temp_store = FILE`
- `page_size = 4096`
- `cache_spill = FALSE`
...
Documents are sorted by total score and the top N are returned. Snippets are generated by locating query tokens in the stored document body, with the stored `excerpt` as a fallback.
...
- **temp_store = FILE**: keeps transient data on disk to minimize RAM use; can increase I/O under heavy sorts. ([https://www.sqlite.org/tempfiles.html](https://www.sqlite.org/tempfiles.html))
