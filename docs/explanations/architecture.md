...
- **Routes**: `/mcp` serves MCP traffic, `/healthz` provides constant-time process liveness, `/health` reports tenant residency, and `/{tenant}/sync/*` exposes scheduler introspection guarded by operation mode.
